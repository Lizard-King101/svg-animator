...
export class RuntimeScene {
    ...
    write(targetId, property, value) {
        ...
        if (point && target.path)
            return this.writePathPoint(target, point[1], point[2], value);
        ...
        if (gradient)
            return this.writeGradient(target, gradient, value);
    ...
    writeGeometryPosition(target, axis, value) {
        ...
        if (target.path)
            forEachPathPoint(target.path.contours, (point) => point[axis] += delta);
    ...
    writeGeometrySize(target, axis, value) {
        if (target.type !== "rectangle" && target.type !== "ellipse")
            return false;
    ...
    transformAttachedGeometry(target, affine) {
        ...
        Object.values(target.paints).forEach((paint) => {
            if (!paint || paint.kind !== "gradient" || paint.units !== "userSpaceOnUse")
                return;
    ...
    writePathPoint(target, id, axis, value) {
        ...
        if (!writePathPoint(target.path.contours, id, axis, value))
            return false;
    ...
    writeGradient(target, match, value) {
        ...
        if (numeric == null)
            return false;
        if (match[5] === "offset")
            stop.offset = clamp01(numeric);
        else
            stop.opacity = clamp01(numeric);
        return true;
    }
    renderGeometry(node, target) {
        const geometryNodes = node.tagName.toLowerCase() === "g"
            ? [...node.querySelectorAll('[data-render-role~="geometry"], [data-render-role~="geometry-effect"]')]
            : [node];
        if (target.path) {
            ...
            if (reveal.length)
                reveal.forEach((item) => setAttribute(item, "stroke-dashoffset", round(1 - progress)));
            else
    ...
    renderAppearance(node, target) {
        ...
        if (fill !== undefined)
            this.roleNodes(node, "fill").forEach((item) => writePaint(item, "fill", fill, "none"));
        if (stroke !== undefined)
            this.roleNodes(node, "stroke").forEach((item) => writePaint(item, "stroke", stroke, null));
        if (color !== undefined)
            writePaint(node, "fill", color, "#000000");
        if (target.stroke) {
    ...
    sampleMotion(target) {
        ...
        if (!path?.path || path.id === target.id)
            return undefined;
        ...
        for (const segment of segments) {
            if (consumed + segment.length >= desired) {
                selected = segment;
                amount = segment.length ? (desired - consumed) / segment.length : 0;
                break;
            }
...
function writePaint(node, attribute, paint, fallback) {
    ...
    if (!paint) {
        ...
        setAttribute(node, opacityAttribute, paint.opacity < 0.9999 ? round(paint.opacity) : null);
    }
}
function writePathPoint(contours, id, axis, value) {
    ...
    if (numeric == null)
        return false;
    let found = false;
    forEachPathPoint(contours, (point) => { if (point.id === id) {
        point[axis] = numeric;
        found = true;
    } });
    return found;
...
function forEachPathPoint(contours, callback) {
    contours.forEach((contour) => contour.lines.forEach((line) => {
        ...
        if (line.controlStart)
            callback(line.controlStart);
        if (line.controlEnd)
            callback(line.controlEnd);
    }));
}
...
function flattenRuntimePath(contours, matrix) {
    return contours.flatMap((contour) => contour.lines).flatMap((line) => {
        ...
        if (line.type !== "bezier" || !line.controlStart || !line.controlEnd)
            return [segment(applyMatrix(matrix, start.x, start.y), applyMatrix(matrix, end.x, end.y))];
...
function segment(start, end) { return { start, end, length: Math.hypot(end.x - start.x, end.y - start.y) }; }
function identity() { return { a: 1, b: 0, c: 0, d: 1, e: 0, f: 0 }; }
function tupleMatrix(value) { return { a: value[0], b: value[1], c: value[2], d: value[3], e: value[4], f: value[5] }; }
function translation(x, y) { return { a: 1, b: 0, c: 0, d: 1, e: x, f: y }; }
function scale(x, y) { return { a: x, b: 0, c: 0, d: y, e: 0, f: 0 }; }
function rotation(degrees) { const r = degrees * Math.PI / 180, c = Math.cos(r), s = Math.sin(r); return { a: c, b: s, c: -s, d: c, e: 0, f: 0 }; }
function multiply(left, right) { return { a: left.a * right.a + left.c * right.b, b: left.b * right.a + left.d * right.b, c: left.a * right.c + left.c * right.d, d: left.b * right.c + left.d * right.d, e: left.a * right.e + left.c * right.f + left.e, f: left.b * right.e + left.d * right.f + left.f }; }
function invert(matrix) { const d = matrix.a * matrix.d - matrix.b * matrix.c; return Math.abs(d) < 1e-6 ? identity() : { a: matrix.d / d, b: -matrix.b / d, c: -matrix.c / d, d: matrix.a / d, e: (matrix.c * matrix.f - matrix.d * matrix.e) / d, f: (matrix.b * matrix.e - matrix.a * matrix.f) / d }; }
function applyMatrix(matrix, x, y) { return { x: matrix.a * x + matrix.c * y + matrix.e, y: matrix.b * x + matrix.d * y + matrix.f }; }
function matrixToSvg(matrix) { return [matrix.a - 1, matrix.b, matrix.c, matrix.d - 1, matrix.e, matrix.f].every((v) => Math.abs(v) < 1e-6) ? null : `matrix(${[matrix.a, matrix.b, matrix.c, matrix.d, matrix.e, matrix.f].map(round).join(" ")})`; }
function rotateVector(x, y, degrees) { const r = degrees * Math.PI / 180, c = Math.cos(r), s = Math.sin(r); return { x: x * c - y * s, y: x * s + y * c }; }
function setAttribute(node, name, value) { if (value == null || value === "")
    node.removeAttribute(name);
else
    node.setAttribute(name, String(value)); }
function assignNumber(target, key, value) { if (value == null)
    return false; target[key] = value; return true; }
...
function attributeSelectorValue(value) { return value.replace(/\\/g, "\\\\").replace(/"/g, '\\"'); }
function isIdentityTransform(value) { return value.every((item, index) => Math.abs(item - [1, 0, 0, 1, 0, 0][index]) < 1e-6); }
