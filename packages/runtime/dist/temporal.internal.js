...
export function evaluateTemporalCubic(time, timeA, timeB, timeC, timeD, valueA, valueB, valueC, valueD) {
...
export function solveTemporalCubicPosition(time, timeA, timeB, timeC, timeD) {
    ...
    for (let index = 0; index < 5; index++) {
        ...
        if (Math.abs(error) < 1e-7 || Math.abs(derivative) < 1e-9)
            break;
    ...
    for (let index = 0; index < 14; index++) {
        ...
        if (Math.abs(sampledTime - time) < 1e-7)
            break;
        if (sampledTime < time)
            low = position;
        else
...
export function evaluateTemporalCubicArray(time, coefficients, offset = 0) {
    return evaluateTemporalCubic(time, coefficients[offset], coefficients[offset + 1], coefficients[offset + 2], coefficients[offset + 3], coefficients[offset + 4], coefficients[offset + 5], coefficients[offset + 6], coefficients[offset + 7]);
}
//# sourceMappingURL=temporal.internal.js.map