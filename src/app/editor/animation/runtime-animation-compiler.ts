...
function supportedProperty(property: string): boolean {
    return /^(transform\.(translateX|translateY|scaleX|scaleY|rotation|originX|originY)|opacity|visible|settings\.(fill|stroke|stroke_width)|path\.drawProgress|motion\.(progress|rotateToPath|offsetAngle|offsetX|offsetY)|path\.points\.[^.]+\.(x|y)|settings\.(fill|stroke)\.gradient\.(x1|y1|x2|y2|cx|cy|r|fx|fy|stops\..+\.(offset|color|opacity)))$/.test(property);
}
