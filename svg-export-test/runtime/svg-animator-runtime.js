...
var SVGAnimatorRuntime = (() => {
  ...
  function evaluateRuntimeTrack(track, time) {
    ...
    if (mode === "temporal") {
      ...
      for (let index = 0; index < 16; index++) {
        const sampledTime = ((coefficients[offset] * position + coefficients[offset + 1]) * position + coefficients[offset + 2]) * position + coefficients[offset + 3];
        if (sampledTime < time) low = position;
        else high = position;
        position = (low + high) / 2;
      }
      return ((coefficients[offset + 4] * position + coefficients[offset + 5]) * position + coefficients[offset + 6]) * position + coefficients[offset + 7];
    }
  ...
  var RuntimeScene = class {
    ...
    write(targetId, property, value) {
      ...
      if (point && target.path) return writePathPoint(target.path.contours, point[1], point[2], value);
      ...
      switch (property) {
        ...
        case "geometry.width":
          if (numeric == null || target.type !== "rectangle" && target.type !== "ellipse") return false;
          target.geometry.width = Math.max(1, numeric);
          return true;
        case "geometry.height":
          if (numeric == null || target.type !== "rectangle" && target.type !== "ellipse") return false;
          target.geometry.height = Math.max(1, numeric);
          return true;
        ...
        case "transform.originX":
          return assignNumber(target.transform, "originX", numeric);
        case "transform.originY":
          return assignNumber(target.transform, "originY", numeric);
        case "opacity":
    ...
    renderGeometry(node, target) {
      ...
      if (target.path) {
        ...
        const reveal = node.querySelectorAll('[data-render-role~="reveal"]');
    ...
    renderAppearance(node, target) {
      ...
      if (target.stroke) {
        ...
        node.querySelectorAll('[data-render-role~="reveal"]').forEach((item) => setAttribute(item, "stroke-width", target.stroke.width * 4));
      }
    }
  ...
  function bootstrapAnimatedSVG(options = {}) {
    ...
    if (!payload?.textContent) throw new RuntimePlayerError("invalid-bundle", "Animated SVG contains no embedded runtime bundle.");
    let bundle;
    try {
      bundle = JSON.parse(payload.textContent);
    } catch (error) {
    ...
    return createPlayer(root, bundle, { autoPlay: true, ...playerOptions });
  }
